...
import type { ReceitaPreparoPelePaciente } from "@workspace/db";
...
/**
 * Armazenamento do PDF de **receita de preparo da pele** da paciente, em Object
 * Storage PRIVADO do Supabase, num bucket PRÓPRIO — separado dos demais — para
 * organizar melhor:
 *
 * - `SUPABASE_STORAGE_BUCKET_RECEITAS_PELE` (obrigatório; nome do bucket privado)
 *
...
function config(): StorageConfig | null {
  ...
  const bucket = process.env.SUPABASE_STORAGE_BUCKET_RECEITAS_PELE?.trim();
...
export function storageReceitasPeleConfigurado(): boolean {
  return config() !== null;
}
 ...
 */
export async function uploadReceitaPreparoPele(params: {
  pacienteId: number;
  buffer: Buffer;
}): Promise<string> {
  const cfg = config();
  ...
  try {
    ...
    if (!resp.ok) {
      ...
      logger.warn(
        ...
        "Falha ao subir receita de preparo da pele para o Supabase Storage",
      );
    ...
    logger.warn({ err }, "Erro ao subir receita de preparo da pele para o Supabase");
    throw new StorageIndisponivelError(err);
  }
}

/** Busca o objeto no bucket de receitas; lança ObjectNotFoundError em 404. */
async function fetchReceita(objectKey: string): Promise<Response> {
  const cfg = config();
...
/** Apaga o objeto do bucket de receitas, tolerando ausência (idempotente). */
export async function apagarReceitaPreparoPeleObjeto(
  objectKey: string,
): Promise<void> {
  const cfg = config();
  if (!cfg) return;
  try {
    ...
    if (!resp.ok && resp.status !== 404) {
      ...
      logger.warn(
        { status: resp.status, detalhe },
        "Falha ao apagar objeto da receita de preparo da pele",
      );
    }
  } catch (err) {
    logger.warn({ err }, "Erro ao apagar objeto da receita de preparo da pele");
  }
}

/**
 * Faz o stream do PDF da receita de volta ao cliente. Espelha `servirDocumento`,
 * mas lê do bucket de receitas. `?download=1` força o download como anexo.
 */
export async function servirReceitaPreparoPele(
  receita: Pick<
  ...
  try {
    const resposta = await fetchReceita(receita.objectPath);

    const disposicao = opts.download ? "attachment" : "inline";
    const nomeOriginal = receita.nomeArquivo?.trim() || "receita-preparo-pele.pdf";
    ...
    if (!resposta.body) {
      res.status(502).json({
        message: "Receita indisponível no momento. Tente novamente.",
      });
      return;
    }
    ...
    nodeStream.on("error", (err) => {
      logger.warn({ err }, "Falha ao ler receita de preparo da pele do armazenamento");
      if (!res.headersSent) {
        res.status(502).json({
          message: "Receita indisponível no momento. Tente novamente.",
        });
    ...
    logger.warn({ err }, "Falha ao servir receita de preparo da pele da paciente");
    res.status(502).json({
      message: "Receita indisponível no momento. Tente novamente.",
    });
  }
